...
/**
 * <b>类</b>条件概率计算 <h3>类条件概率</h3> P(x<sub>j</sub>|c<sub>j</sub>)=(
 * N(X=x<sub>i</sub>, C=c<sub>j </sub>)+1 ) <b>/</b> ( N(C=c<sub>j</sub>)+M+V ) <br>
 * 其中，N(X=x<sub>i</sub>, C=c<sub>j</sub>）表示类别c<sub>j</sub>中包含属性x<sub>
 * i</sub>的训练文本数量；N(C=c<sub>j</sub>)表示类别c<sub>j</sub>中的训练文本数量；M值用于避免
 * N(X=x<sub>i</sub>, C=c<sub>j</sub>）过小所引发的问题；V表示类别的总数。 <h3>条件概率</h3> <b>定义</b>
 * 设A, B是两个事件，且P(A)>0 称<br>
...
public class ClassConditionalProbability
{
    ...
    public static double calculatePxc(String x, String c)
    {
        ...
        double V = tdm.getTraningWordsClassificationsCount();
        ret = (Nxc + 1) / (Nc + M + V);
        return ret;
    }
}
